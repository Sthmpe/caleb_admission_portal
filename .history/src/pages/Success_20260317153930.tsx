...
const Success = () => {
  ...
  useEffect(() => {
    // If there's no reference in the URL, they shouldn't be here
    if (!paymentReference) {
      setError("No payment reference found. Please contact support if you have been debited.");
      setIsLoading(false);
      return;
    }

    // Ping the backend to check if the Webhook has processed the payment and generated the PIN
    const verifyPayment = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/pay/verify?ref=${paymentReference}`);
        if (!res.ok) throw new Error("Payment verification pending or failed.");
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center font-sans p-4">
        ...
        <p className="text-sm text-gray-500 mt-2 text-center">Please wait while we securely generate your Application PIN.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center font-sans p-4">
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-xl max-w-md w-full text-center border-t-4 border-green-500">
        ...
        <p className="text-sm text-gray-500 mb-6 md:mb-8">Welcome, {pinData.name}. Your application fee has been received.</p>
