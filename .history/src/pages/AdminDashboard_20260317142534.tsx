...
const AdminDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 font-sans pb-12 overflow-x-hidden">
      <div className="max-w-6xl mx-auto space-y-6">
        
        {/* HEADER */}
        <div className="bg-slate-900 text-white p-4 md:p-6 rounded-xl shadow-md flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-3">
            <Settings className="w-6 h-6 text-blue-400 shrink-0" aria-hidden="true" />
            <h1 className="text-lg md:text-2xl font-bold truncate">Admin Control Center</h1>
          </div>
          <Link to="/" className="text-sm text-slate-300 hover:text-white flex items-center gap-1 transition-colors bg-slate-800 px-3 py-1.5 rounded-lg whitespace-nowrap">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          <div className="bg-white p-5 md:p-6 rounded-xl shadow-sm flex items-center gap-4 overflow-hidden">
            <div className="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center shrink-0">
              <Users className="w-6 h-6 text-blue-600" aria-hidden="true" />
            </div>
            <div className="min-w-0">
              <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider truncate">Total Applicants</p>
              <p className="text-xl md:text-2xl font-black text-gray-900 truncate">{statsTotal}</p>
            </div>
          </div>
          <div className="bg-white p-5 md:p-6 rounded-xl shadow-sm flex items-center gap-4 overflow-hidden">
            ...
            <div className="min-w-0">
              <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider truncate">Est. Revenue</p>
              <p className="text-xl md:text-2xl font-black text-gray-900 truncate">₦{statsRevenue.toLocaleString()}</p>
            </div>
          </div>
          <div className="bg-white p-5 md:p-6 rounded-xl shadow-sm flex items-center gap-4 overflow-hidden">
            ...
            <div className="min-w-0">
              <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider truncate">Active Agents</p>
              <p className="text-xl md:text-2xl font-black text-gray-900 truncate">{statsAgents}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="p-5 md:p-8">
              ...
              <div className="space-y-4">
                ...
                <div className={`p-4 rounded-lg border transition-all ${settings?.status === 'open' ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-green-300'}`}>
                  <div className="flex items-start sm:items-center justify-between cursor-pointer" onClick={() => handleStatusChange('open')}>
                    <div className="flex items-start sm:items-center gap-4">
                      <CheckCircle className={`w-8 h-8 shrink-0 mt-1 sm:mt-0 ${settings?.status === 'open' ? 'text-green-600' : 'text-gray-400'}`} aria-hidden="true" />
                      <div>
                        <h3 className="font-bold text-gray-900">Open Admissions</h3>
                        <p className="text-xs md:text-sm text-gray-500">Portal is active and accepting payments via Monnify.</p>
                      </div>
                    </div>
                  </div>
                  {settings?.status === 'open' && (
                    <div className="mt-4 pt-4 border-t border-green-200">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <label className="flex items-center gap-2 bg-white p-2 rounded border border-green-200 cursor-pointer hover:bg-green-100 overflow-hidden">
                          <input type="checkbox" checked={settings.programs.ug} onChange={() => handleProgramToggle('ug')} className="w-4 h-4 text-green-600 shrink-0" />
                          <span className="text-xs md:text-sm font-semibold truncate">Undergraduate</span>
                        </label>
                        <label className="flex items-center gap-2 bg-white p-2 rounded border border-green-200 cursor-pointer hover:bg-green-100 overflow-hidden">
                          <input type="checkbox" checked={settings.programs.pg} onChange={() => handleProgramToggle('pg')} className="w-4 h-4 text-green-600 shrink-0" />
                          <span className="text-xs md:text-sm font-semibold truncate">Postgraduate</span>
                        </label>
                        <label className="flex items-center gap-2 bg-white p-2 rounded border border-green-200 cursor-pointer hover:bg-green-100 overflow-hidden">
                          <input type="checkbox" checked={settings.programs.jupeb} onChange={() => handleProgramToggle('jupeb')} className="w-4 h-4 text-green-600 shrink-0" />
                          <span className="text-xs md:text-sm font-semibold truncate">JUPEB</span>
                        </label>
                      </div>
                    </div>
                  )}
                ...
                <div className={`p-4 rounded-lg border transition-all ${settings?.status === 'coming_soon' ? 'border-yellow-500 bg-yellow-50' : 'border-gray-200 hover:border-yellow-300'}`}>
                  <div className="flex items-start sm:items-center justify-between cursor-pointer" onClick={() => handleStatusChange('coming_soon')}>
                    <div className="flex items-start sm:items-center gap-4">
                      <Clock className={`w-8 h-8 shrink-0 mt-1 sm:mt-0 ${settings?.status === 'coming_soon' ? 'text-yellow-600' : 'text-gray-400'}`} aria-hidden="true" />
                      <div>
                        <h3 className="font-bold text-gray-900">Coming Soon</h3>
                        <p className="text-xs md:text-sm text-gray-500">Show waiting message.</p>
                      </div>
                    </div>
                  </div>
                ...
                {/* CLOSED */}
                <div className={`p-4 rounded-lg border flex items-start sm:items-center justify-between cursor-pointer transition-all ${settings?.status === 'closed' ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-red-300'}`} onClick={() => handleStatusChange('closed')}>
                  <div className="flex items-start sm:items-center gap-4">
                    <XCircle className={`w-8 h-8 shrink-0 mt-1 sm:mt-0 ${settings?.status === 'closed' ? 'text-red-600' : 'text-gray-400'}`} aria-hidden="true" />
                    <div>
                      <h3 className="font-bold text-gray-900">Close Admissions</h3>
                      <p className="text-xs md:text-sm text-gray-500">Shut down the portal entirely.</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden h-fit flex flex-col">
            <div className="p-5 md:p-6 border-b border-gray-100 bg-blue-50/50">
            ...
            <div className="p-4 flex-grow">
              ...
              {agents.length === 0 ? (
                ...
                <div className="space-y-3 min-h-[180px]">
                  {currentAgents.map((agent, index) => (
                    <div key={agent.id || index} className="bg-gray-50 p-3 rounded-lg border border-gray-200 text-sm overflow-hidden">
                      <div className="flex justify-between items-start mb-1">
                        <span className="font-bold text-gray-900 truncate pr-2">{agent.name}</span>
                        <button onClick={() => copyToClipboard(agent.code)} className="text-blue-600 hover:text-blue-800 shrink-0" title="Copy Code" aria-label="Copy Code">
                          <Copy className="w-4 h-4" aria-hidden="true" />
                        </button>
                      </div>
                      <p className="text-xs text-gray-500 mb-2 truncate">{agent.phone}</p>
                      <div className="flex justify-between items-center bg-white border border-gray-200 rounded px-2 py-1">
                        <span className="font-mono font-bold text-blue-700 tracking-wide text-xs truncate mr-2">{agent.code}</span>
                        <span className="text-[10px] font-bold bg-green-100 text-green-800 px-2 py-0.5 rounded-full shrink-0 whitespace-nowrap">{agent.uses} Uses</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {agents.length > 0 && (
                <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-bold text-gray-500 whitespace-nowrap">Page {agentPage} of {totalAgentPages}</span>
                  <button 
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-5 md:p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs md:text-sm text-gray-500">Monitor Monnify payments and biodata submissions.</p>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider border-b border-gray-200">
                  <th className="p-4 font-semibold whitespace-nowrap">Applicant Name</th>
                  <th className="p-4 font-semibold whitespace-nowrap">Program</th>
                  <th className="p-4 font-semibold whitespace-nowrap">App PIN</th>
                  <th className="p-4 font-semibold whitespace-nowrap">Agent Ref</th>
                ...
                {currentApplicants.length === 0 ? (
                  ...
                  currentApplicants.map((applicant) => (
                    <tr key={applicant.id || applicant.pin} className="hover:bg-blue-50/50 transition-colors">
                      <td className="p-4 font-bold text-gray-900 text-sm whitespace-nowrap">{applicant.name}</td>
                      <td className="p-4 text-sm text-gray-600 whitespace-nowrap">{applicant.program}</td>
                      <td className="p-4 whitespace-nowrap"><span className="font-mono text-xs font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded border border-gray-200">{applicant.pin}</span></td>
                      <td className="p-4 text-sm text-gray-500 whitespace-nowrap">{applicant.refCode}</td>
          ...
          {totalApplicants > 0 && (
            <div className="p-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-700 px-2 whitespace-nowrap">Page {currentPage} of {totalPages}</span>
                <button 
