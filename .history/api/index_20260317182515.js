...
app.post(['/api/biodata/submit', '/biodata/submit'], async (req, res) => {
  ...
  if (!pin || !biodata) return res.status(400).json({ error: 'Missing Data' });

  try {
    const applicantRef = db.collection('applicants').doc(pin);
    
    // We use { merge: true } so we don't accidentally delete their 
    // email or payment info while saving the biodata
    ...
    res.status(200).json({ success: true, message: 'Application Completed' });
  } catch (error) {
...
app.post(['/api/biodata/generate-pdf', '/biodata/generate-pdf'], async (req, res) => {
  ...
  try {
    // 1. Get the student's data from Firebase
    const doc = await db.collection('applicants').doc(pin).get();
    ...
    // 2. THIS IS WHERE WE CALL THE ACTUAL PDF FILE
    // It looks for a folder named 'assets' in your root directory
    const templatePath = path.join(process.cwd(), 'assets', 'Admissionform1stMay2020.pdf');
    
    if (!fs.existsSync(templatePath)) {
      console.error("Missing PDF at:", templatePath);
    ...
    // Get Page 1 and Page 3
    const pages = pdfDoc.getPages();
    const firstPage = pages[0];
    const thirdPage = pages[2]; // Index 2 is the 3rd page

    // 4. WRITE TEXT DIRECTLY ONTO THE PDF
    const drawOpts = { size: 11, font, color: rgb(0, 0, 0) };

    // --- PAGE 1 ---
    firstPage.drawText(pin, { x: 440, y: 765, ...drawOpts }); // Form Number
    firstPage.drawText("2026/2027", { x: 440, y: 700, ...drawOpts }); // Session
    firstPage.drawText(bio.jambRegNo || '', { x: 120, y: 550, ...drawOpts }); 
    firstPage.drawText(String(bio.academicScore || ''), { x: 120, y: 535, ...drawOpts });
    firstPage.drawText((bio.lastName || '').toUpperCase(), { x: 120, y: 505, ...drawOpts });
    firstPage.drawText((bio.firstName || '').toUpperCase(), { x: 120, y: 480, ...drawOpts });

    // --- PAGE 3 ---
    if (thirdPage) {
      thirdPage.drawText(bio.sponsorName || '', { x: 120, y: 650, ...drawOpts });
      thirdPage.drawText(data.name || '', { x: 150, y: 150, ...drawOpts }); // Signature name
    }

    // 5. Save the modified PDF and send it to the frontend
